...
class ofSynthStimberg {
    
public:
    
    ofSynthStimberg(int numOsc);
    ~ofSynthStimberg(){};
...
inline
float ofSynthStimberg::getFrequency(bool isFundamental) const {
    
    return ( isFundamental ? fundamentalFreq : currentFreq );
}

inline
float ofSynthStimberg::getAmplitude(int oscNum) const {
    
    return sineOsc[oscNum].getAmplitude();
}

inline
float ofSynthStimberg::getSampleRate() const {
    
    return sampleRate;
}

inline
void ofSynthStimberg::setSampleRate(const float& smplRate) {
