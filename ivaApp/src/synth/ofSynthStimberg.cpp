...
#include "synth/ofSynthStimberg.h"


ofSynthStimberg::ofSynthStimberg(int numOsc)
{
    assert(currentFreq == fundamentalFreq);
    setupOscillators(numOsc);
}

void ofSynthStimberg::updateSoundBuffer(ofSoundBuffer& outBuffer)
{    
...
void ofSynthStimberg::setFrequency(int oscNum, const float& freq) {
...
void ofSynthStimberg::setAmplitude(int oscNum, const float& amp) {
...
void ofSynthStimberg::setupOscillators(int numOsc) {
...
void ofSynthStimberg::updateOscillatorFreq(const float& freq) {
